...
def main() -> int:
    ...
    # a daily staging series, long enough to exercise the sheet's per-field cap
    st.record_staging(run_id, [
        {"project": "Ether.fi", "name": field, "value": 1.23 + i / 100, "date": str((TODAY - pd.Timedelta(days=i)).date()),
         "source": "FIXTURE:dune:8683038", "tier": 4, "note": "candidate for the 30-day column; used by nothing"}
        for field in ("agg_14", "agg_30") for i in range(40)
    ])
